...
export function listCoffees() {
  return sqlite.prepare(`SELECT ${coffeeSelect} FROM coffees ORDER BY brand ASC, blend ASC`).all() as Coffee[];
}

export function getCoffee(id: string) {
  return sqlite.prepare(`SELECT ${coffeeSelect} FROM coffees WHERE id = ?`).get(id) as Coffee | undefined;
}
...
export function createCoffee(coffee: CoffeeInput) {
  ...
  sqlite
    .prepare(
      `INSERT INTO coffees (id, brand, blend, roast_level, roast_date, origin, process, notes, created_at, updated_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`
    )
    .run(
      coffee.id,
...
export function updateCoffee(id: string, coffee: CoffeeUpdate) {
  sqlite
    .prepare(
      `UPDATE coffees
       SET brand = ?, blend = ?, roast_level = ?, roast_date = ?, origin = ?, process = ?, notes = ?, updated_at = ?
       WHERE id = ?`
    )
    .run(
      coffee.brand,
